...
import 'wlDetails_controller.dart';
...
class WatchlistSocketController {
  ...
  Future<void> startBroadcast({required String clientId}) async {
    final wlState = ref.read(wlDetailsControllerProvider).value;
    ...
    final keys = wlState.items
        .map((item) {
      final segId = getSegId(item.exchange, item.segment);
      if (segId == -1) {
...
final watchlistSocketControllerProvider = Provider<WatchlistSocketController>((ref) {
